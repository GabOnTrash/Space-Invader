...
void Star::InitStars(int tipo)
{
    for (int i = 0; i < STARS_NUM; i++)
    {
        ...
        if (tipo == 1)
        {
            stars[i].speed.x = GetRN<float>(50.f, 100.f) / 10.f;
            stars[i].speed.y = -GetRN<float>(50.f, 100.f) / 10.f;
        }
        else
        {
            stars[i].speed.x = 0;
    ...
    for (int i = 0; i < STARS_NUM; i++)
    {
        ...
        if (tipo == 1)
        {
            if (stars[i].position.x > ViewPort::BASE_WIDTH + 10)
                stars[i].position.x = GetRN<float>(-50.f, 0.f);

            if (stars[i].position.y < -10)
                stars[i].position.y = GetRN<float>(static_cast<float>(ViewPort::BASE_HEIGHT), static_cast<float>(ViewPort::BASE_HEIGHT + 50));
        }
        else
        {
