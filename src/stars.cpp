...
void Star::InitStars(int tipo)
{
    ...
    {
        stars[i].position = { GetRN<float>(0, BASE_WIDTH), GetRN<float>(0, BASE_HEIGHT)};
        if (tipo == 1)
        {
...
void Star::updateStars(float deltaT, int tipo)
{
    for (int i = 0; i < STAR_NUM; i++)
    {
        ...
        if (tipo == 1)
        {
            if (stars[i].position.x > BASE_WIDTH + 10)
                stars[i].position.x = GetRN<float>(-50, 0);

            if (stars[i].position.y < -10)
                stars[i].position.y = GetRN<float>(BASE_HEIGHT, BASE_HEIGHT + 50);
        }
        else
        {
            if (stars[i].position.y > BASE_HEIGHT + 10)
            {
                stars[i].position.y = GetRN<float>(-50.0f, 0.0f);
                stars[i].position.x = GetRN<float>(3, BASE_WIDTH - 3);
            }       
        }
    }
}
